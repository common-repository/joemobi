...
class JOEMOBI_API_Author {
  ...
  // Note:
  //   JOEMOBI_API_Author objects can include additional values by using the
  //   author_meta query var.
  
  function JOEMOBI_API_Author($id = null) {
    if ($id) {
      $this->id = (int) $id;
    } else {
  ...
  function set_author_meta() {
    global $joemobi_api;
    if (!$joemobi_api->query->author_meta) {
      return;
    }
    ...
    $vars = explode(',', $joemobi_api->query->author_meta);

...
class JOEMOBI_API_Post {
  
  // Note:
  //   JOEMOBI_API_Post objects must be instantiated within The Loop.
  ...
  function JOEMOBI_API_Post($wp_post = null) {
    if (!empty($wp_post)) {
  ...
  function save($values = null) {
    ...
    if (!empty($values['author'])) {
      $author = $joemobi_api->introspector->get_author_by_login($values['author']);
    ...
    if (isset($values['categories'])) {
      ...
      foreach ($categories as $category_slug) {
        ...
        $category = $joemobi_api->introspector->get_category_by_slug($category_slug);
    ...
    if (!empty($_FILES['attachment'])) {
      ...
      $this->attachments[] = new JOEMOBI_API_Attachment($attachment_id);
    ...
    $this->set_value('comment_count', @count((array)$this->comments));
  ...
  function set_value($key, $value) {
    global $joemobi_api;
    if ($joemobi_api->include_value($key)) {
      $this->$key = $value;
    } else {
      unset($this->$key);
    }
	...
	function set_content_value() {
	  global $joemobi_api;
	  if ($joemobi_api->include_value('content')) {
	    $content = get_the_content($joemobi_api->query->read_more);
	    ob_start();
  ...
  function set_categories_value() {
    global $joemobi_api;
    if ($joemobi_api->include_value('categories')) {
      $this->categories = array();
      if ($wp_categories = get_the_category($this->id)) {
        foreach ($wp_categories as $wp_category) {
          $category = new JOEMOBI_API_Category($wp_category);
  ...
  function set_tags_value() {
    global $joemobi_api;
    if ($joemobi_api->include_value('tags')) {
      $this->tags = array();
      if ($wp_tags = get_the_tags($this->id)) {
        foreach ($wp_tags as $wp_tag) {
          $this->tags[] = new JOEMOBI_API_Tag($wp_tag);
        }
      }
    } else {
      unset($this->tags);
  ...
  function set_author_value($author_id) {
    global $joemobi_api;
    if ($joemobi_api->include_value('author')) {
      $this->author = new JOEMOBI_API_Author($author_id);
    } else {
      unset($this->author);
    }
  }
  ...
  function set_comments_value() {
    global $joemobi_api;
    if ($joemobi_api->include_value('comments')) {
      $this->comments = $joemobi_api->introspector->get_comments($this->id);
    } else {
      $this->comments = array();
    }
  }
  ...
  function set_attachments_value() {
    global $joemobi_api;
    if ($joemobi_api->include_value('attachments')) {
      $this->attachments = $joemobi_api->introspector->get_attachments($this->id);
    } else {
  ...
  function set_thumbnail_value() {
    global $joemobi_api;
    if (!$joemobi_api->include_value('thumbnail') ||
  ...
  function set_custom_fields_value() {
    global $joemobi_api;
    if ($joemobi_api->include_value('custom_fields') &&
        $joemobi_api->query->custom_fields) {
      $keys = explode(',', $joemobi_api->query->custom_fields);
  ...
  function get_thumbnail_size() {
    global $joemobi_api;
    if ($joemobi_api->query->thumbnail_size) {
      return $joemobi_api->query->thumbnail_size;

...
class JOEMOBI_API_Query {
  ...
  function get_legacy_controller($json) {
    ...
    if ($json == 'submit_comment') {
      if ($joemobi_api->controller_is_active('respond')) {
        return 'respond';
      } else {
        $joemobi_api->error("The 'submit_comment' method has been removed from the Core controller. To use this method you must enable the Respond controller from WP Admin > Settings > Joemobi.");
      }
    } else if ($json == 'create_post') {
      if ($joemobi_api->controller_is_active('posts')) {
        return 'posts';
      } else {
        $joemobi_api->error("The 'create_post' method has been removed from the Core controller. To use this method you must enable the Posts controller from WP Admin > Settings > Joemobi.");
      }
    } else {
      return 'core';
    }
  ...
  function get_method($controller) {
    ...
    //     * http://example.org/2009/11/10/hello-world/?joemobi=1 (get_post)
    //     * http://example.org/2009/11/?joemobi=1 (get_date_posts)
    //     * http://example.org/category/foo?joemobi=1 (get_category_posts)
    
    $method = $this->get('joemobi');
    ...
    if (empty($method)) {
      ...
      return false;
    } else if (method_exists("JOEMOBI_API_{$controller}_Controller", $method)) {
      ...
      if (is_search()) {
        ...
        if (empty($_GET['joemobi'])) {
          $joemobi_api->error("Uknown method '$method'.");
        }

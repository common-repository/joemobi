...
class JOEMOBI_API_Introspector {
  ...
  public function get_posts($query = false, $wp_posts = false) {
    ...
    while (have_posts()) {
      ...
      if ($wp_posts) {
        ...
        $output[] = new JOEMOBI_API_Post($post);
      }
    }
    return $output;
  }
  ...
  public function get_current_category() {
    global $joemobi_api;
    extract($joemobi_api->query->get(array('id', 'slug', 'category_id', 'category_slug')));
    if ($id || $category_id) {
      if (!$id) {
        $id = $category_id;
      }
  ...
  public function get_current_tag() {
    global $joemobi_api;
    extract($joemobi_api->query->get(array('id', 'slug', 'tag_id', 'tag_slug')));
    if ($id || $tag_id) {
      if (!$id) {
        $id = $tag_id;
      }
  ...
  public function get_current_author() {
    global $joemobi_api;
    extract($joemobi_api->query->get(array('id', 'slug', 'author_id', 'author_slug')));
    if ($id || $author_id) {
      if (!$id) {
        $id = $author_id;
      }
  ...
  public function get_author_by_id($id) {
    ...
    return new JOEMOBI_API_Author($id);
  }
  ...
  public function get_comments($post_id) {
    ...
    foreach ($wp_comments as $wp_comment) {
      $comments[] = new JOEMOBI_API_Comment($wp_comment);
    }
    return $comments;
  }
  ...
  public function get_attachments($post_id) {
    ...
    if (!empty($wp_attachments)) {
      foreach ($wp_attachments as $wp_attachment) {
        $attachments[] = new JOEMOBI_API_Attachment($wp_attachment);
      }
    }
    return $attachments;
  }
  ...
  public function attach_child_posts(&$post) {
    ...
    foreach ($wp_children as $wp_post) {
      $post->children[] = new JOEMOBI_API_Post($wp_post);
    }
  ...
  protected function get_category_object($wp_category) {
    ...
    return new JOEMOBI_API_Category($wp_category);
  }
  ...
  protected function get_tag_object($wp_tag) {
    ...
    return new JOEMOBI_API_Tag($wp_tag);
  }
  ...
  protected function set_posts_query($query = false) {
    ...
    if ($joemobi_api->query->page) {
      $query['paged'] = $joemobi_api->query->page;
    }
    
    if ($joemobi_api->query->count) {
      $query['posts_per_page'] = $joemobi_api->query->count;
    }
    
    if ($joemobi_api->query->post_type) {
      $query['post_type'] = $joemobi_api->query->post_type;
    }
    //var_dump($query);
    if (!empty($query)) {
      query_posts($query);
